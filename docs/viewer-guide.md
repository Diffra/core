...
The review interface includes four synchronized comparison modes:
...
* **Component filter and search**: Real-time filtering by story name, component group, or status (changed, added, passed).
* **Keyboard shortcuts**: Navigate stories and toggle view modes using number keys and arrow keys.
* **Cross-branch baseline jump**: Clickable header badges linking to the latest baseline branch report.
