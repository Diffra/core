...
## Comparison engine and inspection modes

Diffra provides five synchronized comparison modes mounted inside standardized framed canvas-backdrop stages:

1. **Pixel movement diff highlight (`Movement`) [Default]**:
   * Automatically isolates changed and shifted pixels in high-visibility neon green (`#00FF66`) against a desaturated candidate backdrop using `<ScanEye />`.
2. **Split view (`Split`)**: Side-by-side synchronized view with proportional zoom and zero duplicated pane headers.
3. **Swipe slider (`Swipe`)**: Horizontal interactive divider slider.
4. **Onion skin (`Onion`)**: Smooth opacity crossfade.
5. **Diff mask (`Mask`)**: Perceptual delta-E color mask with 2Hz blink mode and bounding box highlights.
